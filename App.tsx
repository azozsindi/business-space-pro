    ...
    <div className="min-h-screen flex bg-[#f8fafc] text-slate-800 overflow-hidden font-['IBM_Plex_Sans_Arabic']">
      <aside className="w-80 bg-[#0f172a] flex flex-col p-8 space-y-10 z-10 shadow-2xl text-slate-400">
        <div className="flex items-center gap-4 px-2">
          <div className="w-14 h-14 bg-indigo-600 rounded-[1.8rem] flex items-center justify-center text-white shadow-2xl shadow-indigo-600/30">
            <LayoutGrid size={32} />
          </div>
          <div>
            <span className="text-2xl font-black text-white leading-tight block">Business</span>
            <span className="text-xs font-bold text-indigo-400 uppercase tracking-widest">Space Pro</span>
          </div>
        </div>

        <nav className="space-y-3 flex-1 overflow-y-auto pr-2 custom-scrollbar">
          <button onClick={() => setView('calendar')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black transition-all ${view === 'calendar' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/40' : 'hover:bg-slate-800 hover:text-white'}`}>
            <CalendarIcon size={20} /> التقويم التفاعلي
          </button>
          <button onClick={() => setView('projects')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black transition-all ${view === 'projects' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/40' : 'hover:bg-slate-800 hover:text-white'}`}>
            <TrendingUp size={20} /> إحصائيات الأداء
          </button>
          <button onClick={() => setView('notifications')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black transition-all ${view === 'notifications' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/40' : 'hover:bg-slate-800 hover:text-white'}`}>
            <Bell size={20} /> سجل النشاطات
          </button>
          {user.permissions.canManageUsers && (
            <button onClick={() => setView('admin')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black transition-all ${view === 'admin' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/40' : 'hover:bg-slate-800 hover:text-white'}`}>
              <Users size={20} /> إدارة الفريق
            </button>
          )}
          {isSuperAdmin && (
            <button onClick={() => setView('spaces')} className={`w-full flex items-center gap-4 px-6 py-4 rounded-2xl font-black transition-all ${view === 'spaces' ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-600/40' : 'hover:bg-slate-800 hover:text-white'}`}>
              <ShieldAlert size={20} /> إدارة المساحات
            </button>
          )}
        </nav>
        ...
        <header className="h-24 bg-white/70 backdrop-blur-xl border-b border-slate-200 flex items-center justify-between px-12 sticky top-0 z-20">
          <div className="relative w-96 group">
            ...
            <input 
              ...
              className="bg-slate-100 border-none rounded-[1.5rem] py-4 pr-14 pl-6 w-full text-sm font-bold outline-none focus:ring-4 focus:ring-indigo-500/5 focus:bg-white transition-all shadow-inner text-right"
            />
          </div>
          ...
          {view === 'calendar' ? (
            <div className="max-w-7xl mx-auto">
              <div className="flex flex-col md:flex-row md:items-end justify-between gap-8 mb-16">
                <div className="text-right">
                  ...
                  <p className="text-lg text-slate-400 font-bold">إحصائية الشهر: <span className="text-indigo-600">{stats.completedTasks} مكتملة</span> من أصل {stats.totalTasks}</p>
                </div>
                  ...
                  return (
                    <button
                      ...
                      className={`relative flex flex-col p-6 bg-white aspect-square rounded-[2.8rem] border-2 transition-all duration-500 overflow-hidden group 
                        ${isMatch ? 'ring-4 ring-yellow-400 border-yellow-400 scale-105 z-20 shadow-2xl' : 'border-slate-50 hover:border-indigo-200 hover:-translate-y-2 hover:shadow-2xl'}`}
                    >
                      ...
                      <div className="flex flex-wrap gap-1.5 mt-4">
                        ...
                        {data.media.length > 0 && <div className="w-2 h-2 rounded-full bg-indigo-500" />}
            ...
            <div className="max-w-6xl mx-auto py-8 text-right">
               ...
               <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-4 gap-8">
                  ...
                  <div className="bg-white p-10 rounded-[3rem] shadow-xl border border-slate-100 flex flex-col items-center text-center">
                    <div className="w-20 h-20 bg-indigo-50 text-indigo-600 rounded-3xl flex items-center justify-center mb-6"><Paperclip size={40} /></div>
               ...
               <div className="mt-12 bg-[#0f172a] p-12 rounded-[3.5rem] text-white shadow-2xl relative overflow-hidden">
                  <div className="absolute top-0 left-0 w-1/2 h-full bg-indigo-500/10 blur-[100px]"></div>
            ...
            <div className="max-w-4xl mx-auto py-8 text-right">
               ...
               <div className="space-y-6">
                  {notifications.filter(n => n.spaceId === user.spaceId).length === 0 && (
                  ...
                  {notifications.filter(n => n.spaceId === user.spaceId).map(n => (
                    <div key={n.id} className="bg-white p-8 rounded-[2.2rem] border border-slate-100 shadow-xl flex flex-row-reverse items-center gap-6 group hover:border-indigo-500 transition-all">
                       <div className="w-16 h-16 rounded-2xl bg-slate-50 flex items-center justify-center text-slate-400 group-hover:bg-indigo-50 group-hover:text-indigo-600 transition-all"><Info size={28} /></div>
                       <div className="flex-1">
                          <p className="text-lg font-bold text-slate-800">{n.message}</p>
                          <div className="flex flex-row-reverse items-center gap-3 mt-2">
                            <span className="text-[10px] font-black text-indigo-500 bg-indigo-50 px-3 py-1 rounded-full">{n.user}</span>
